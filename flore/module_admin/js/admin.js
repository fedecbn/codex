...
//------------------------------------------------------------------------------ UI / Boutons / User
	...
	$( "#admin-user-add-button" )
        ...
        .click(function() {
            metaForm ("Ajouter un utilisateur",670,400,'#admin-user-dialog',"user-form.php","user-submit.php",user_oTable,"",$(this).attr('name'));
		});
...
{ type: "select", values:['CBN Alpin','CBN de Bailleul','CBN de Brest','CBN de Corse','CBN de Franche-Comté','CBN des Pyrénées et de Midi-Pyrénées','CBN du Bassin Parisien','CBN du Massif central','[Guyane]','[Grand Est]','[Guadeloupe]','[Martinique]','FCBN','inconnu','CBN Méditerranéen de Porquerolles','CBN Sud-Atlantique','CBN Mascarin']},                    { type: "text" },                                           // Login
    ...
    $('#admin-user-liste').on("click",".admin-user-edit", function ($e) {
        metaForm ("Modifier un utilisateur",670,400,'#admin-user-dialog',"user-form.php","user-submit.php",user_oTable,$(this).attr('id'),$(this).attr('name'));
        return (false);
	});
